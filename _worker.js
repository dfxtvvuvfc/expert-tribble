                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            try {
                                ...
                                // Track the latest browser subscription-page access.
                                // This is the IP/time of the person opening the subscription page.
                                const viewerIp = request.headers.get("cf-connecting-ip") || request.headers.get("x-forwarded-for") || null;
                                targetUser.lastIp = viewerIp || targetUser.lastIp || null;
                                targetUser.lastConnectedAt = Date.now();
                                try {
                                    await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
                                } catch (e) {}
                                const embeddedSubData = {
                                    username: targetUser.name,
                                    data_limit: limitTotalBytes,
                                    used_traffic: totalBytesUsed,
                                    expire: expiryMs ? Math.floor(expiryMs / 1000) : null,
                                    created_at: targetUser.createdAt || null,
                                    updated_at: targetUser.updatedAt || targetUser.createdAt || null,
                                    online_at: targetUser.lastConnectedAt,
                                    last_ip: targetUser.lastIp,
                                    device_limit: targetUser.maxConfigs || null,
                                    subscription_url: syncNormal,
                                    raw_subscription_url: syncRaw,
                                    links: [],
                                    apps: []
                                };
                                html = html.replace(/__SUB_DATA__/g, JSON.stringify(embeddedSubData).replace(/</g, "\u003c"));
